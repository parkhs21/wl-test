...
public class ApiPayload<T> {
    ...
    public static <T> ApiPayload<T> onSuccess(BaseStatus status, T data) {
        return new ApiPayload<>(true, status.getReason().code(), status.getReason().message(), data);
    }

    // 실패
    public static <T> ApiPayload<T> onFailure(BaseStatus status, T data) {
        return new ApiPayload<>(false, status.getReason().code(), status.getReason().message(), data);
    }
